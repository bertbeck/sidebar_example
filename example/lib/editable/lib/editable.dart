...
class Editable extends StatefulWidget {
  ...
  /// [onEditRow] callback is triggered when a [saveButton] is pressed.
  /// returns only values if row is edited, otherwise returns a string ['no edit']
  final ValueChanged<dynamic>? onEditRow;
...
class EditableState extends State<Editable> {
  ...
  Widget build(BuildContext context) {
    ...
    /// Builds editIcon widget
    Widget _editIcon(index) {
      return Flexible(
        fit: FlexFit.loose,
        flex: 6,
        child: Visibility(
          ...
          child: Container(
            ...
            child: IconButton(
              ...
              icon: Icon(
                ...
                size: widget.editIconSize,
              ),
              onPressed: () {
                // int rowIndex = editedRows.indexWhere(
                //     (element) => element['row'] == index ? true : false);
                print('edit row $index');
                widget.onEditRow!(index);
                return;
                // if (rowIndex != -1) {
                //   widget.onRowSaved!(editedRows[rowIndex]);
                // } else {
    ...
    List<Widget> _tableHeaders() {
      int colIndex = -1;
      return List<Widget>.generate(columnCount! + (widget.showEditIcon ? 1 : 0),
          (_colIndex) {
        if (widget.showEditIcon && _colIndex == 0) {
          return iconColumn(
              widget.showEditIcon, widget.thPaddingTop, widget.thPaddingBottom);
        }
        colIndex++;
        return THeader(
            widthRatio: columns![colIndex]['widthFactor'] != null
                ? columns![colIndex]['widthFactor'].toDouble()
                : widget.columnRatio,
    ...
    List<Widget> _tableRows() {
      return List<Widget>.generate(rowCount!, (index) {
        ...
        return Row(
          ...
          children: List.generate(columnCount! + (widget.showEditIcon ? 1 : 0),
              (_colIndex) {
            var ckeys = [];
            var cwidths = [];
            var ceditable = <bool>[];
            ...
            return RowBuilder(
              ...
              widthRatio: cwidths[colIndex].toDouble(),
              isEditable: ceditable[colIndex],
              ...
              onChanged: (value) {
                ...
                if (result != -1) {
                  editedRows[result][ckeys[colIndex]] = value;
                } else {
                  var temp = {};
                  temp['row'] = index;
                  temp[ckeys[colIndex]] = value;
                  editedRows.add(temp);
                }
              },
            );
